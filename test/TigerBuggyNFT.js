...
describe("TigerBuggyNFT contract", function () {
  ...
  beforeEach(async function () {
    ...
    tiger = await tigerFactory.deploy(
      artist.address,
  ...
  it("only owner can put tiger up for sale", async function () {
    await expect(
      tiger.connect(bob).putUpForSale(13, ethers.utils.parseEther("1"))
  ...
  it("owner can put tiger up for sale", async function () {
    await tiger
      .connect(bob)
      .buyTiger(13, { value: ethers.utils.parseEther("1") });
  ...
  it("someone can buy a tiger that is for sale", async function () {
    ...
    expect(
      await tiger.connect(alice).tigerByOwnerAndIndex(bob.address, 0)
    ).to.equal(13);
  });
  ...
  it("purchaser can resell a tiger", async function () {
    ...
    await tiger
      .connect(alice)
      .buyTiger(13, { value: ethers.utils.parseEther("2") });
  ...
  it("multiple tiger purchases can be tracked", async function () {
    await tiger
      .connect(bob)
      .buyTiger(3, { value: ethers.utils.parseEther("1") });
    await tiger
      .connect(bob)
      .buyTiger(13, { value: ethers.utils.parseEther("1") });
    await tiger
      .connect(bob)
      .buyTiger(23, { value: ethers.utils.parseEther("1") });
  ...
  it("multiple tiger purchases and resales can be tracked", async function () {
    ...
    await tiger
      .connect(alice)
      .buyTiger(3, { value: ethers.utils.parseEther("2") });
    await tiger
      .connect(alice)
      .buyTiger(13, { value: ethers.utils.parseEther("2") });
    expect(await tiger.getBalance(bob.address)).to.equal(1);
    expect(await tiger.tigerByOwnerAndIndex(bob.address, 0)).to.equal(23);
    await expect(tiger.tigerByOwnerAndIndex(bob.address, 1)).to.be.revertedWith(
  ...
  it("tiger should show as no longer for sale after it's been bought", async function () {
    await tiger
      .connect(bob)
      .buyTiger(38, { value: ethers.utils.parseEther("1") });
  ...
  it("can't buy a tiger that is not for sale", async function () {
    await tiger
      ...
      .buyTiger(38, { value: ethers.utils.parseEther("1") });
    await expect(
      tiger.connect(bob).buyTiger(38, { value: ethers.utils.parseEther("1") })
  ...
  it("can't buy a tiger that has just been bought by someone else", async function () {
    ...
    await tiger
      ...
      .buyTiger(13, { value: ethers.utils.parseEther("1") });
    await expect(
      tiger.connect(alice).buyTiger(13, { value: ethers.utils.parseEther("1") })
    ).to.be.revertedWith("not for sale");
  });
  ...
  it("sellers can withdraw funds", async function () {
    ...
    await tiger
      .connect(alice)
      .buyTiger(13, { value: ethers.utils.parseEther("15") });
